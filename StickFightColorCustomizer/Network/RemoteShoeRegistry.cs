using System.Collections.Generic;

namespace StickFightColorCustomizer.Network
{
    public static class RemoteShoeRegistry
    {
        private static readonly Dictionary<ulong, string> BySteamId = new Dictionary<ulong, string>();
        private static readonly Dictionary<byte, string> ByPlayerId = new Dictionary<byte, string>();
        private static readonly Dictionary<ulong, int> HashBySteamId = new Dictionary<ulong, int>();

        public static void SetBySteam(ulong steamId, string shoeId)
        {
            if (steamId == 0)
            {
                return;
            }

            shoeId = ShoeSyncCodec.Decode(shoeId);
            int hash = shoeId != null ? shoeId.GetHashCode() : 0;
            int prev;
            if (HashBySteamId.TryGetValue(steamId, out prev) && prev == hash)
            {
                return;
            }

            HashBySteamId[steamId] = hash;
            BySteamId[steamId] = shoeId;
        }

        public static void Set(byte playerId, string shoeId)
        {
            shoeId = ShoeSyncCodec.Decode(shoeId);
            ByPlayerId[playerId] = shoeId;
        }

        public static bool TryGetBySteam(ulong steamId, out string shoeId)
        {
            return BySteamId.TryGetValue(steamId, out shoeId);
        }

        public static bool TryGet(byte playerId, out string shoeId)
        {
            return ByPlayerId.TryGetValue(playerId, out shoeId);
        }

        public static void RemoveSteam(ulong steamId)
        {
            if (steamId == 0)
            {
                return;
            }

            BySteamId.Remove(steamId);
            HashBySteamId.Remove(steamId);
        }

        public static void RemoveSlot(byte playerId)
        {
            ByPlayerId.Remove(playerId);
        }

        public static void Clear()
        {
            BySteamId.Clear();
            ByPlayerId.Clear();
            HashBySteamId.Clear();
        }
    }
}
